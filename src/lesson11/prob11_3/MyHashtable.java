...
public class MyHashtable implements Iterable {
	...
	private void rehash() {
		...
		temptable = new MyHashtable(tableSize * 2);
		
		var it = h.iterator();
		while(it.hasNext()) {
			Entry temp = getEntry(it.next());
			temptable.put(temp.key, temp.value);
			h.remove(temp.key);
		}
